...
import discord
from discord import errors, File, Message, Embed, TextChannel, Member, Intents, app_commands
...
# file that stores the anime IDs being tracked with /track
tracked_file = os.path.join(root_dir, "tracked.json")
...
class GlobalsType:
    ...
    tracker_period = 60 * 60 * 24  # repost tracked countdowns once a day
    feed_channel: Any = None
    nsfw_feed_channel: Any = None
    tracker_channel: Any = None
    old_db: Any = None
    tracked_store: Any = None
...
async def print_loop() -> None:
    ...
    Globals.feed_channel = get(channels, name="announcements")
    Globals.nsfw_feed_channel = get(channels, name="announcements")
    if Globals.feed_channel is None:
        logger.critical("Couldn't find the 'announcements' channel")
    if Globals.nsfw_feed_channel is None:
        logger.critical("Couldn't find the 'announcements' channel")
    # falls back to the announcements channel if there's no dedicated 'tracker' channel
    Globals.tracker_channel = get(channels, name="tracker") or Globals.feed_channel
    Globals.old_db = OldDatabase(filepath=old_db_file)
    Globals.tracked_store = TrackedStore(filepath=tracked_file)
    # copy the globally-registered slash commands into this guild so they're
    # available instantly, instead of waiting up to an hour for a global sync
    client.tree.copy_global_to(guild=guilds[0])
    await client.tree.sync(guild=guilds[0])
...
@log
async def post_tracker_updates() -> None:
    """Posts a fresh countdown embed to the tracker channel for every tracked anime"""
    tracked = await Globals.tracked_store.read()
    for mal_id_str in list(tracked.keys()):
        try:
            countdown = await compute_countdown(int(mal_id_str), logger=logger)
            await Globals.tracker_channel.send(embed=build_tracker_embed(countdown))
        except Exception as e:
            logger.warning(f"Failed to post tracker update for {mal_id_str}: {e}")
        await sleep(1)  # a little breathing room between entries


async def tracker_loop() -> None:
    assert Globals.tracker_channel is not None
    assert Globals.tracked_store is not None
    while not client.is_closed():
        await post_tracker_updates()
        await sleep(Globals.tracker_period)


class AnimeSearchSelect(discord.ui.Select):
    """Dropdown of Jikan search results, shown after /track <name>"""

    def __init__(self, results: List[Dict[str, Any]]):
        self.results_by_id = {str(r["mal_id"]): r for r in results if r.get("mal_id")}
        options = [
            discord.SelectOption(
                label=english_title(r)[:100],
                description=f"MAL ID {r['mal_id']} - {r.get('year') or 'Year unknown'}",
                value=str(r["mal_id"]),
            )
            for r in results
            if r.get("mal_id")
        ...
        super().__init__(
            placeholder="Select the anime to track...",
            options=options,
            min_values=1,
            max_values=1,
        )
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        chosen = self.results_by_id[self.values[0]]
        mal_id = int(chosen["mal_id"])
        title = english_title(chosen)
        await Globals.tracked_store.add(mal_id, title)
        await interaction.response.edit_message(
            content=f"Now tracking **{title}** (MAL ID {mal_id}). "
            f"Countdown updates will post in {Globals.tracker_channel.mention} "
            f"about once a day.",
            view=None,
        )
        try:
            countdown = await compute_countdown(mal_id, logger=logger)
            await Globals.tracker_channel.send(embed=build_tracker_embed(countdown))
        except Exception as e:
            logger.warning(f"Failed to post initial tracker update for {mal_id}: {e}")


class AnimeSearchView(discord.ui.View):
    def __init__(self, results: List[Dict[str, Any]]):
        super().__init__(timeout=60)
        self.add_item(AnimeSearchSelect(results))


@client.tree.command(
    name="track", description="Track the next-episode/next-season countdown for an anime"
)
@app_commands.describe(name="Anime name to search for")
async def track_slash(interaction: discord.Interaction, name: str) -> None:
    await interaction.response.defer(thinking=True)
    results = await search_anime(name, limit=5, logger=logger)
    if not results:
        await interaction.followup.send(f"Couldn't find any anime matching '{name}'.")
        return
    lines = "\n".join(
        f"- {english_title(r)} ({r.get('year') or 'year unknown'})" for r in results
    )
    await interaction.followup.send(
        f"Found these matches for '{name}', pick one to track:\n{lines}",
        view=AnimeSearchView(results),
    )


@client.tree.command(name="untrack", description="Stop tracking an anime")
@app_commands.describe(mal_id="The MAL id of the anime to stop tracking (see /tracked)")
async def untrack_slash(interaction: discord.Interaction, mal_id: int) -> None:
    removed = await Globals.tracked_store.remove(mal_id)
    if removed:
        await interaction.response.send_message(f"Stopped tracking MAL id {mal_id}.")
    else:
        await interaction.response.send_message(
            f"MAL id {mal_id} wasn't being tracked. Use /tracked to see the current list."
        )


@client.tree.command(name="tracked", description="List anime currently being tracked")
async def tracked_slash(interaction: discord.Interaction) -> None:
    tracked = await Globals.tracked_store.read()
    if not tracked:
        await interaction.response.send_message(
            "Nothing is being tracked yet. Use /track <name> to add one."
        )
        return
    lines = "\n".join(f"- {title} (MAL ID {mid})" for mid, title in tracked.items())
